...
class GpsFollower(Node):
    def __init__(self):
        ...
        csv_path = os.path.join(script_dir, 'Z_trajectory7_4to2in10s_freq100.csv')
        self.vertical_tracejtory = pd.read_csv(csv_path).values
        self.trajectory_counter = 0
        self.trajectory_hold_height = 2.0
        self.car_height = 0.53
    ...
    def attitude_loop(self):
        ...
        while rclpy.ok():
            ...
            if in_trajectory_phase:
                ...
                if self.trajectory_counter<len(self.vertical_tracejtory):
                
                    desired_height = self.vertical_tracejtory[self.trajectory_counter]
            ...
            if use_pose is not None:
                self.distance_pid.kp = 0.5  # Lower gain when tag is seen
                ...
                if desired_height is None:
                    altitude_error = self.fixed_altitude - (0.53 + use_pose.pose.position.y)
                else:
            ...
            if log_counter % 10 == 0:
                ...
                self.get_logger().info(
                    ...
                    f"TagHeight: {tag_height_str} | Bearing Car: {self.heading_log:.2f}° | TagLateral: {tag_lateral_str} | DemandedHeight: {desired_height} |"
                )
            log_counter += 1
