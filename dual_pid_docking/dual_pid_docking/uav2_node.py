...
# REMOVE SimulatedMovingTarget class and any references to it
# ... [KEEP IMPORTS + PID class + euler_to_quaternion as-is]
...
class UAVFollower(Node):
    def __init__(self):
        ...
        self.lateral_pid = PID(0.0013, 0.0014, 0.16, integral_limit=1.0)
    ...
    def attitude_loop(self):
        while rclpy.ok():
            ...
            # If AprilTag seen and trajectory not yet triggered
            if use_cv and not self.trajectory_triggered:
                self.trajectory_triggered = True
                self.trajectory_start_time = now
                self.get_logger().info("🎯 AprilTag seen — starting descent trajectory")

            if self.trajectory_triggered:
                elapsed = now - self.trajectory_start_time
                if elapsed < self.trajectory_duration:
                    factor = elapsed / self.trajectory_duration
                ...
                elif elapsed < self.trajectory_duration + self.hold_duration:
                    self.fixed_altitude = self.low_altitude
                    self.target_distance = self.low_distance
                else:
                    # Optional: revert back or hold
                    pass
            ...
            throttle_cmd = self.base_throttle + self.airspeed_pid.update(airspeed_error, dt)
